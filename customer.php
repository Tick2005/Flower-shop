...
if (isset($_POST['add_to_cart']) && $user_id) {
    ...
    $stmt = $conn->prepare("SELECT * FROM cart WHERE user_id = ? AND pid = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt->bind_param("iisdsi", $user_id, $pid, $name, $price, $quantity, $image);
        if ($stmt->execute()) {
    ...
    <header class="header bg-green-50 shadow-md sticky top-0 z-50">
        <div class="container mx-auto px-4 py-4 flex justify-between items-center">
            <div class="flex items-center space-x-6">
                <a href="customer.php" class="text-2xl font-bold text-green-600">Flower Shop</a>
